...
// const fitur_tambahan = readFileSync("fitur_tambahan.sql", "utf8");
...
query = "";
for (let i = 0; i < 100; ++i) {
	query += objectToInsertQuery("pengunjung", generator.pengunjung());
}
const listIdPengunjung = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
for (const idPengunjung of listIdPengunjung) {
	...
	for (let i = 0; i < count; ++i)
		query += objectToInsertQuery(
			"nomor_telepon_pengunjung",
...
for (const idPengunjung of listIdPengunjung) {
	...
	for (let i = 0; i < count; ++i)
		query += objectToInsertQuery("masukan", generator.masukan(idPengunjung));
}
await connection.query(query);
...
query = "";
for (let i = 0; i < 20; ++i) {
	query += objectToInsertQuery("wahana", generator.wahana());
}
const listIdWahana = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
const listIdGrupAntrian = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
query = "";
for (let i = 0; i < 20; ++i) {
	query += objectToInsertQuery("shift", generator.shift());
}
const listIdShift = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
query = "";
for (let i = 0; i < 20; ++i) {
	query += objectToInsertQuery("pegawai", generator.pegawai());
}
const listIdPegawai = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
for (const idShift of listIdShift) {
	...
	const randomized = faker.helpers.arrayElements(listIdPegawai, {
		min: listIdWahana.length,
	});
...
query = "";
for (let i = 0; i < 20; ++i) {
	query += objectToInsertQuery("souvenir", generator.souvenir());
}
const listIdSouvenir = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
const listIdTransaksi = (await connection.query(query)).map((v) =>
	Number(v.insertId),
);
...
for (const idTransaksi of listIdTransaksi) {
	const count = faker.number.int({
		min: 0,
		max: Math.min(5, listIdSouvenir.length),
	});
